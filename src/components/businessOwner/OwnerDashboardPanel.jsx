...
export default function Dashboard({ baseData }) {
    // NEW: real local state instead of relying only on props
    const [businessData, setBusinessData] = useState(baseData || null);
    ...
    useEffect(() => {
        ...
        if (!ownerId) {
            console.error("❌ No OwnerID found in loggedUser.");
    ...
    if (!businessData) return <div>No data available.</div>;
...
${topRecs || "No recommendations generated."}
    ...
    return (
        <div className="dashboard-container">
            ...
            {/* ERRORS */}
            {shareError && <div className="alert alert-danger py-2 small mb-2">{shareError}</div>}
            {shareSuccess && <div className="alert alert-success py-2 small mb-2">{shareSuccess}</div>}
